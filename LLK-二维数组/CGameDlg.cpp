...
void CGameDlg::UpdateMap() {
	...
	for (int i = 0; i < nRows; ++i) {
		for (int j = 0; j < nCols; ++j) {
			...
			assert(GAMETOP_Y == nTop);
			int nElemVal = m_GameC.GetElement(i, j);
			m_dcMem.BitBlt(nLeft + j * ELEM_WIDTH, nTop + i * ELEM_HEIGHT, ELEM_WIDTH, ELEM_HEIGHT, &m_dcMask, 0, nElemVal * ELEM_HEIGHT, SRCPAINT);
			m_dcMem.BitBlt(nLeft + j * ELEM_WIDTH ,nTop + i * ELEM_HEIGHT, ELEM_WIDTH, ELEM_HEIGHT, &m_dcElement, 0, nElemVal * ELEM_HEIGHT, SRCAND);
		}
	}
	Invalidate(FALSE);
}
...
void CGameDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (m_bFirstPoint) {
		...
		Vertex avPath[MAX_VERTEX_NUMS];
		int nVexnum = 0;
		std::cout << m_GameC.m_graph.GetVertex(m_GameC.m_svSelFst) << " " << m_GameC.m_graph.GetVertex(m_GameC.m_svSelSec) << std::endl;
